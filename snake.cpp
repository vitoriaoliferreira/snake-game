...
ALLEGRO_FONT *font20 = NULL;         // carrega a fonte do placar
    ...
    while (!sair)
    {
        ...
        if (!intro)
        {
            ...
            if (redraw && al_is_event_queue_empty(event_queue))
            {
                ...
                al_draw_bitmap(mapa, 0, 0, 0);
                int cont = -1; //contador para inicializar o vetor, descontando o pixel das margens
                bool comeu = false;//booleano da snake comer uma cereja incia falso
                for (int i = 0; i < 26; i++)
                    for (int j = 0; j < 26; j++)
                        if (andou[i][j] > 0 && andou[i][j] >= passo - corpo - 1)
                        {
                            cont++;//aumenta o contador a cada "passo"
                           i= vet[cont * 2 + 1];
                           j=  vet[cont * 2];
                            if (i == ifruta && j == jfruta) // cobrinha comeu a fruta
                                comeu = true;
                            al_draw_bitmap(quad, i * q, j * q, 0); // desenha quadrado
                        }
                if (comeu)
                { //caso uma cereja seja colhida
                    ...
                    int k;
                    while (teste)
                    {//enquanto o teste nao der negativo uma nova cereja nao eh criada  
                ...
                if (contespecial == looping-1)
                {
                    ...
                    while (teste)
                    {
                        ...
                        jespecial = rand() % 23 + 1;
                        for (int k = cont * 2 + 1; k >= 1; k -= 2) //mesmo caso da cereja
                    ...
                    contespecial++; //contador da abelha acumula um valor com inicio 0
                }
                else if (contespecial == looping)//quando o jogo atinge o valor estipulado de "voltas"
                {
                    especial = al_load_bitmap("abelha.tga"); // cria o inseto
                    al_draw_bitmap(especial, jespecial * q, iespecial * q, 0);
                    if (j == jespecial && i == iespecial)//se a posicao da cabeca da cobra for a mesma da abelha
                    {
                        ...
                        contespecial = 0; //zera o contador das abelhas
                    }
                }
                else
                {
                ...
                al_draw_text(font20, al_map_rgb(217, 4, 4), 250, 480, ALLEGRO_ALIGN_CENTER, texto.c_str());
